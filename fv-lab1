...
def get_rating_bar(row):
    try:
        ...
        v = (pd.to_numeric(row.get('Professionalita', 5))*0.25 + pd.to_numeric(row.get('Esperienza', 5))*0.20) # Semplificato per brevità
        return "🟩"*int(v/2), v
    except: return "⬜"*5, 0.0
...
def genera_pdf(data_str, schieramento, split_list, lista_assenti):
    ...
    # Definiamo l'ordine personalizzato
    ordine_speciale = ["Hotel Castello", "Hotel Castello 4 Piano", "MACRO: PALME & GARDEN"]
    
    # Separiamo gli hotel ordinati da quelli restanti
    mappa_res = {r['Hotel']: r for r in schieramento}
    final_ordered = []
    for o in ordine_speciale:
        if o in mappa_res: final_ordered.append(mappa_res[o])
    for r in schieramento:
        if r['Hotel'] not in ordine_speciale: final_ordered.append(r)
    ...
    p.setFont("Helvetica", 11); p.drawString(60, y-50, f"Personale: {', '.join(split_list) if split_list else 'Nessuno assegnato'}")
    p.save(); buffer.seek(0)
    return buffer

df = load_data()

# --- SIDEBAR (CORRETTA CON MULTI-SELECT) ---
with st.sidebar:
    ...
    with st.form("form_v7"):
        ...
        # --- QUI LA MODIFICA: MULTISELECT ---
        z_attuali = str(curr['Zone_Padronanza']).split(", ") if curr is not None else []
        f_zn = st.multiselect("Zone di Padronanza", lista_hotel, default=[z for z in z_attuali if z in lista_hotel])
        
        if st.form_submit_button("💾 SALVA"):
            nuova = {"Nome": f_n, "Ruolo": f_r, "Zone_Padronanza": ", ".join(f_zn), "Part_Time": 0, "Conteggio_Spezzati": 0}
...
with t3:
    st.header("🚀 Planning")
    data_p = st.date_input("Data:", datetime.now(), key="v7_d")
    assenti = st.multiselect("🛌 Assenti:", nomi_db, key="v7_a")
    
    # (Logica inserimento AI/FI omessa per brevità, rimane identica a prima)
    # ... Inserimento cur_inp ...

    if st.button("🚀 GENERA SCHIERAMENTO", key="v7_btn"):
        attive = df[~df['Nome'].isin(assenti)].copy()
        # Calcolo Spezzati
        pool_spl = attive[(attive['Ruolo'] == 'Cameriera')].head(4)['Nome'].tolist()
        
        # Generazione schieramento (Logica flessibile già presente)
        # ... Logica ris ...
        
        st.session_state['res_v7'] = ris 
        st.session_state['spl_v7'] = pool_spl # Questo ora popola il PDF

    if 'res_v7' in st.session_state:
        # Visualizzazione expander e tasto scarica PDF
        # ...
        if st.button("🧊 SCARICA PDF FINALE"):
            pdf = genera_pdf(data_p.strftime("%d/%m/%Y"), st.session_state['res_v7'], st.session_state['spl_v7'], assenti)
