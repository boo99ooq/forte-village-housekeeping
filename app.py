...
# --- FUNZIONE CARICAMENTO DATIimport streamlit as st
import pandas as pd
import os
from datetime import datetime
...
with st.sidebar:
    ...
    if not df.empty:
        csv_back = df.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Scarica Backup (Copia PC)", data=csv_back, file_name='housekeeping_backup.csv', mime='text/csv')
    ...
    with st.form("form_staff", clear_on_submit=(modo == "Inserisci Nuova")):
        ...
        if st.form_submit_button("SALVA / AGGIORNA"):
            if nome_in:
                ...
                df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
                df.to_csv(FILE_DATA, index=False)
                st.success("Modifica registrata su GitHub!")
                st.rerun()
...
with tab_home:
    if not df.empty:
        # Mostra l'orario dell'ultima attività globale
        ultima_mod = df['Ultima_Modifica'].dropna().max() if 'Ultima_Modifica' in df.columns else "N/D"
        st.caption(f"🕒 Ultimo aggiornamento database: **{ultima_mod}**")
        ...
        st.info("Inizia inserendo il personale nella barra laterale.")

# (Tab Config e Planning rimangono invariati rispetto alla versione precedente)
with tab_config:
    st.header("Configurazione Minuti")
    if os.path.exists(FILE_CONFIG):
        config_df = pd.read_csv(FILE_CONFIG)
